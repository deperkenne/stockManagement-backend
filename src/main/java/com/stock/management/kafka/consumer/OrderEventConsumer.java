...
public class OrderEventConsumer {
    ...
    public void onOrderReceived(
            @Payload List<OrderReceivedEvent> events,
        ...
        log.info("[KAFKA] Received batch of {} orders — first: topic={} partition={} offset={}",
                events.size(), topics.get(0), partitions.get(0), offsets.get(0));
        try {
            eventHandler.handleOrderReceivedBatch(events);
            ack.acknowledge();
        } catch (Exception ex) {
            log.error("[KAFKA] Batch processing failed size={} : {}", events.size(), ex.getMessage(), ex);
            throw ex;
        }
    }
